...
const Home = () => {
  ...
  const fetchNFTsForCollection = async (startTokenId = 0) => {
    if (collection.length) {
      var requestOptions = {
        method: "GET",
      };
      const baseURL = `https://eth-mainnet.alchemyapi.io/v2/${api_key}/getNFTsForCollection/`;
      const fetchURL = `${baseURL}?contractAddress=${collection}&startToken=${startTokenId}&withMetadata=${"true"}`;
    ...
    <div className="flex flex-col items-center justify-center py-8 gap-y-3">
      <div className="flex flex-col w-full justify-center items-center gap-y-2">
        ...
        <div className="flex flex-wrap gap-y-12 mt-4 w-5/6 gap-x-2 justify-center">
          {NFTs.length &&
            NFTs.map((nft) => {
              return <NFTCard nft={nft} key={nft.title} />;
            })}
        </div>
        <div className="flex">
          {currentPage > 1 && (
            <button onClick={previousPage}>First Page</button>
          )}
          <div className="ml-8 mr-8">{currentPage}</div>
